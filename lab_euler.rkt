...
;;   (get-vertices empty) -> ()
;;
;;returns a list of all vertices in g (with repeats)
(define (gvHelper g)
  (if (null? g) '() (append (list (car(car g)) (car(cdr(car g)))) (gvHelper (cdr g)))))
...
;;   (degree empty 1) -> 0
;;
;;counts degree of v without adding 2 for a reflexive edge
(define (degreeH G v)
    (if (null? G) 0 (if (in G v) (+ 1 (degreeH (cdr G) v)) (+ 0 (degreeH (cdr G) v)))))

;; Type signature: (degree graph vertex) -> int
(define (degree G v)
  (define n (degreeH G v))
  (if (in? G (list v v)) (+ n 1) n))
...
;;   (all-even? empty) -> #t
;;
;;checks if a given vertice has an even degree
(define (aeHelper g v)
  (even? (degree g v)))

;;iterates through a list of vertices to check if all have even degrees
(define (aeHelper1 g l)
  (if (null? l) #t (and (aeHelper g (car l)) (aeHelper1 g (cdr l)))))

;; Type signature: (all-even? graph) -> bool
(define (all-even? G)
  (aeHelper1 G (get-vertices G)))
...
;;   (adjacent empty 5) -> ()
;;
;;Returns if v is in the first list of l
(define (in l v)
  (or (equal? (car(car l)) v) (equal? (car(cdr(car l))) v)))

;;Returns if list p is in g
(define (in? g p)
  (if (null? g) #f (or (equal? (car g) p) (in? (cdr g) p))))

;;Cons the pairs in G that contain vertex v
(define (adjacentH G v)
    (if (null? G) '() (if (in G v) (cons (car G) (adjacentH (cdr G) v)) (adjacentH (cdr G) v))))

;; Type signature: (adjacent graph vertex) -> vertex-list
(define (adjacent G v)
 (usort(if (in? G (list v v)) (get-vertices (adjacentH G v)) (remove (get-vertices (adjacentH G v)) (list v)))))
...
;;   (connected? empty) -> #t  
;;
;;Returns if a vertex can access all vertices
(define (cHelper G v)
  (equal? (get-vertices G) (usort (dfs G '() (car v)))))

;;Returns whether all vertices are connected
(define (cHelper1 G v)
  (if (null? v) #t (if (cHelper G v) (and #t (cHelper1 G (cdr v))) (and #f (cHelper1 G (cdr v))))))

;; Type signature: (connected? graph) -> bool
(define (connected? G)
  (cHelper1 G (get-vertices G)))
