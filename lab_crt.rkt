...
(define (euclid-gcd a b)
  (if (equal? b 0) a (if (equal? (equal? b 0) #f) (euclid-gcd b (modulo a b)) (euclid-gcd a b))))
...
;;takes out the divisor from a list of pairs
(define (crt-helper1 l)
  (if (null? l) '() (cons (car(cdr(car l))) (crt-helper1 (cdr l)))))

;;checks if a and b are pairwise relatively prime
(define (rel-prime? a b) (equal? (euclid-gcd a b) 1))

;;compares the s to every value in L
(define (exist-helper s L)
  (if (null? L) #t (if (rel-prime? s (car L)) (and #t (exist-helper s (cdr L))) (and #f (exist-helper s (cdr L))))))

;;iterates through a list to see if all values are pairwise relatively prime
(define (crt-helper2 x y)
  (if (or (null? x) (null? y)) #t (and (exist-helper (car x) (cdr y)) (crt-helper2 (cdr x) (cdr y)))))

;; Type Signature: (CRT-exists? cong-sys) -> boolean
(define (CRT-exists? cong-sys)
  (define l (crt-helper1 cong-sys))
  (crt-helper2 l l))
...
(define (Mi cong-sys m)(/ m (car(cdr(car cong-sys)))))
(define (bi cong-sys) (car(cdr(car cong-sys))))
(define (ai cong-sys) (car(car cong-sys)))
(define (yi cong-sys m) (mul-inv (Mi cong-sys m) (bi cong-sys)))
(define (X cong-sys m)(if (null? cong-sys) 0 (+ (*(ai cong-sys) (* (Mi cong-sys m) (yi cong-sys m))) (X (cdr cong-sys) m))))
;; Type Signature: (CRT-helper cong-sys int) -> int
(define (CRT-helper cong-sys m)
  (define x (X cong-sys m))
  x) 
...
(define (CRT cong-sys)
  (if (equal? (CRT-exists? cong-sys) #f) -1 (modulo (CRT-helper cong-sys (m cong-sys)) (m cong-sys))))