...
(define (id n)
 (make-set(if (equal? n 0) '() (cons (list n n) (id (- n 1))))))
...
(define (reflexive-closure R n)
  (make-set(if (equal? n 0) R (cons (list n n) (reflexive-closure R (- n 1))))))
...
(define (inverse R)
  (make-set(if (equal? R '()) '() (cons (list (car(cdr(car R))) (car(car R))) (inverse (cdr R))))))
...
(define (relates-to v R)
  (make-set(remove '() (if (equal? R '()) '() (cons (if (equal? v (car(car R))) (car(cdr(car R))) '()) (relates-to v (cdr R)))))))
