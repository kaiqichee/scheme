...
(define (intersection LA LB)
  (if (null? LA) '() (if (member (car LA) LB) (make-set(cons (car LA) (intersection (cdr LA) LB))) (make-set(intersection (cdr LA) LB)))))
...
(define (set-equal? SA SB)
  (define (se-helper SA SB)
  (define c (cond ((null? SA) 0)
      ((member (car SA) SB) (+ 1  (se-helper (cdr SA) SB)))
      (+ 0  (se-helper (cdr SA) SB)))) c)
  (define l (length SA))
  (= l (se-helper SA SB)))
...
(define (set-difference LA LB)
  (make-set(if (null? LA) '() (if (element? (car LA) LB) (set-difference (cdr LA) LB) (cons (car LA) (set-difference (cdr LA) LB))))))
...
(define (disjoint? SA SB)
  (if (null? (intersection SA SB)) #t #f))
...
(define (insert e L)
  (if (member e L) (make-set L) (make-set (cons e L))))
...
(define (remove e S)
  (make-set (if (null? S) '() (if (equal? (car S) e) (remove e (cdr S)) (cons (car S) (remove e (cdr S)))))))
