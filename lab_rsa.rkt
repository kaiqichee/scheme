...
(define (generate p q e)
  ...
  (define d (mul-inv e L))
  (list (list m e) (list m d)))
...
;;
(define (meHelper b e m)
    (if (equal? e 0) 1 (modulo (if (equal? (modulo e 2) 0) (expt (meHelper b (/ e 2) m) 2) (* b (meHelper b (- e 1) m))) m)))
...
(define (encrypt plaintext key)
  (define pt (if (string? plaintext) (string->list plaintext) plaintext))
   (if (null? plaintext) '() (append (list (mod-exp  (char->integer (car pt)) (car(cdr key)) (car key)))
                                               (encrypt (cdr pt) key))))
...
#|
   ...
   -> "RACKET IS GREAT"
|#
(define (decryptHelper ciphertext key)
   (if (null? ciphertext) '()
                    (cons  (integer->char (mod-exp (car ciphertext) (car (cdr key)) (car key))) (decryptHelper (cdr ciphertext) key))))
;; Type signature: (decrypt int-list '(int int)) -> string
(define (decrypt ciphertext key)
  (list->string (decryptHelper ciphertext key)))
...
;; m = 247    e = 23          '(159 80 85 138 128 49 80 91 128 201 80 91 154 237 128 86 43 67) => "Wow, you found me!"
;; m = 145    e = 17          '(23 102 45 80 11 11 52 44 115 2 115 104 55 45 96 2 104 11 102 80) => "Sandeep's shiny head"
;; m = 5963   e = 151         '(1080 4133 5460 4133 10 1117 4133 10 3635 4133 5605 5877 4133 1080 5605) => "4 8 15 16 23 42"