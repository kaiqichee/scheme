...
;;
;;Checks if the second item in an edge is v
(define (indHelper e v)
  (equal? (car(cdr e)) v ))
;; Type signature: (indegree edge-list vertex) -> int
(define (indegree Es v)
  (if (null? Es) 0 (if (indHelper (car Es) v) (+ 1 (indegree (cdr Es) v)) (+ 0 (indegree (cdr Es) v)))))
...
;;   (find-top ankh) -> 5
;;
;;Returns a list of lists, each inner list is (index,indegree)
(define (indList v e count)
  (if (null? v) '() (cons (list count (indegree e (car v))) (indList (cdr v) e (+ count 1)))))
;;Returns the lowest index of the value with indegree 0
(define (find x)
  (if (null? x) (tsort-error) (if (equal? (car(cdr(car x))) 0) (car(car x)) (find (cdr x)))))
;;Takes a list of vertices and an index(count), returns vertice at that index
(define (index v count)
  (if (equal? count 0) (car v) (index (cdr v) (- count 1))))
;; Type signature: (find-top graph) -> vertex
(define (find-top G)
  (define e (get-edges G))
  (define v (get-vertices G))
  (define l (indList v e 0))
  (define ind (find l))
  (index v ind))
...
(define (tsort G)
  (if (null? (car G)) '() (cons (find-top G) (tsort (pop-vertex G (find-top G))))))
