...
(define (pz? p)
  (equal? p z))
;;do i have to account for negative numbers, or will the helper functions help with that/can i use them like this?
...
(define (pred p)
  (if (equal? p z) (eopl:error "pred error: zero has no predecessor!") (car p))) 
...
;; Type signature: (p* peano peano) -> peano
(define (p* a b)
  (if (or (pz? a) (pz? b)) z (if (pz? b) a (p+ a (p* a (pred b))))))
...
;; Type signature: (p^ peano peano) -> peano
(define (p^ a b)
  (if (and (pz? b) (pz? a)) (succ z) (if (or (pz? a)) z (if (pz? b) (succ z) (if (pz? b) a (p* a (p^ a (pred b))))))))
...
(define (p= a b)
  (if (or (pz? a) (pz? b)) (and (pz? a) (pz? b)) (p= (pred a) (pred b))))
...
;; Type signature: (p> peano peano) -> bool
(define (p> a b)
  (if (and (pz? a) (pz? b)) #f (if (or (pz? a) (pz? b)) (and (pz? b) (equal? (pz? a) #f)) (p> (pred a) (pred b)))))
...
;; Type signature: (p>= peano peano) -> bool
(define (p>= a b)
  (if (and (pz? a) (pz? b)) #t (if (or (pz? a) (pz? b)) (and (pz? b) (equal? (pz? a) #f)) (p>= (pred a) (pred b)))))
...
;; Type signature: (p% peano peano) -> peano
(define (p% a b)
  (if (pz? b) (eopl:error "modulo error: mod is zero!") (if (p> b a) a (p% (p- a b) b))))
...
(define (primeHelper p c)
  (if (p= c (succ z)) #t (if (equal? (pz? (p% p c)) #t) #f (primeHelper p (pred c)))))

;; Type signature: (p-prime? peano) -> bool
(define (p-prime? p)
  (if (p>= (succ z) p) #f (if (primeHelper p (pred p)) #t #f)))
